...
public class HrController {
	...
	// 조명재 - 인사 코드 휴직자 조회 
	@RequestMapping("/onLeaveDetail")
	public String onLeaveDetail(HttpServletRequest req, Model model) {
		logger.info("url: 인사 코드 그룹 조회 상세페이지");

		hrService.onLeaveDetail(req, model);

		return "hr/ajax/onLeaveDetail";
	}

	// 조명재 - 인사 코드 퇴직자 조회 
	@RequestMapping("/retiredDetail")
	public String retiredDetail(HttpServletRequest req, Model model) {
		logger.info("url: 인사 코드 그룹 조회 상세페이지");

		hrService.retiredDetail(req, model);

		return "hr/ajax/retiredDetail";
	}
